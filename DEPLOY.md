...
대시보드 상단의 **콜렉터** 행이 이 라벨로 집계하고, 슬레이브 패널 범례도
`bas · 502` 형태가 된다. 상단 `콜렉터` 드롭다운으로 특정 콜렉터만 걸러 볼 수 있다.

### 대시보드 구성

| 행 | 패널 |
|---|---|
| **개요** | 통합 아웃바운드 / 활성 슬레이브 / **유휴 슬레이브** / 슬레이브 CPU / 메모리 / 장비 CPU |
| **콜렉터** | 콜렉터별 아웃바운드(스택) · 콜렉터 현재값(막대) · 콜렉터 서버별 |
| **슬레이브** | 슬레이브별 아웃바운드 · **슬레이브 상태 표** · 인바운드 · CPU/메모리 |
| **장비** | 슬레이브 합계 vs 호스트 NIC · 장비 CPU/메모리 |

두 패널이 특히 쓸모 있다.

- **유휴 슬레이브** — 아웃바운드가 0 인 슬레이브 수. 0 이 아니면 주황색이 된다.
  마스터가 일부 포트에 붙지 않은 상황이 바로 드러난다.
